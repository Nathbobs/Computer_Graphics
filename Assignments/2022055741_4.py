...
def prepare_vao_frame_2(): #newly added(2nd triangle frame)
    # prepare vertex data (in main memory)
    vertices = glm.array(glm.float32,
        # position        # color
...
def main():
    ...
    vao_frame_2 = prepare_vao_frame_2() #newly added
    ...
    while not glfwWindowShouldClose(window):
        ...
        T = np.array([[1., 0., np.sin(t)],
                      [0., 1., .0], #newly added (edited from .2 to .0) this adjusted the height of the triangle.
                      [0., 0., 1.]])
        
        T2 = np.array([[1., 0., .0], #newly added
                      [0., 1., np.sin(t)],
                      [0., 0., 1.]])
        ...
        # current frame: M
        glUniformMatrix3fv(M_loc, 1, GL_TRUE, T) #newly added(changed from M to T - 1st triangle frame)
        ...
        glUniformMatrix3fv(M_loc, 1, GL_TRUE, T2) #newly added(2nd triangle frame)

        # draw triangle w.r.t. the current frame
        glBindVertexArray(vao_triangle)
        glDrawArrays(GL_TRIANGLES, 0, 3)

        glUniformMatrix3fv(M_loc, 1, GL_TRUE, R) #newly added(rotating straight line)    


        # draw current frame
        glBindVertexArray(vao_frame_2) #edited (changed from vao_frame to vao_frame_2)
        glDrawArrays(GL_LINES, 0, 6)
