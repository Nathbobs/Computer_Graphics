...
def key_callback(window, key, scancode, action, mods):
    ...
    if key==GLFW_KEY_ESCAPE and action==GLFW_PRESS:
        ...
        if action==GLFW_PRESS or action==GLFW_REPEAT:
            if key==GLFW_KEY_A: #changed to s and changed the rest too
                alpha += np.radians(10)
            elif key==GLFW_KEY_Z:
                alpha -= np.radians(10) # change += to -=
            elif key==GLFW_KEY_S:
                beta += np.radians(10)
            elif key==GLFW_KEY_X:
                beta -= np.radians(10)
...
def main():
    ...
    while not glfwWindowShouldClose(window):
        ...
        # view matrix
        view_pos = glm.vec3(0,2,8) # camera position
        ...
        # ZYX Euler angles
        #t = glfwGetTime() ##blured out
        g_alpha = glm.radians(alpha) #* np.sin(t) ##edited for xang
        g_beta = glm.radians(beta)
        g_gamma = glm.radians(gamma)
        Rz1 = glm.rotate(g_alpha, (0,0,1))
        Rx = glm.rotate(g_beta, (1,0,0))
        Rz2 = glm.rotate(g_gamma, (0,0,1))
        M = glm.mat4(Rz1 * Rx * Rz2)
