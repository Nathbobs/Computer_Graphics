...
def draw_cube_array(vao, MVP, MVP_loc):
    ...
    for i in range(5):
        for j in range(5):
            for k in range(5):
                MVP_cube = MVP * glm.translate(glm.vec3(1*i, 1*j, 1*k)) * glm.scale(glm.vec3(.5,.5,.5)) #if we delete  * glm.scale(glm.vec3(.5,.5,.5)) all cubes are attached together.
                glUniformMatrix4fv(MVP_loc, 1, GL_FALSE, glm.value_ptr(MVP_cube))
                glDrawArrays(GL_TRIANGLES, 0, 36)
                #means that for each cube, mvp is translated and scaled.
def main():
    # initialize glfw
    if not glfwInit():
        return
    ...
    while not glfwWindowShouldClose(window):
        ...
        P = glm.ortho(-5,5, -5,5, -10,10) #these values can be changed for different results.
